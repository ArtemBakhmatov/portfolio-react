...
const ProjectList = () => {
    ...
    const filterProjects = (items, filter) => {
        switch (filter) {
            case 'Landing':
                return items.filter(item => item.web === 'Landing');
            case 'Wordpress':
                return items.filter(item => item.web === 'Wordpress');
            case 'Store':
                return items.filter(item => item.web === 'Store');
            case 'React':
                return items.filter(item => item.web === 'React');
            case 'Bot':
                return items.filter(item => item.web === 'Bot');
            default:
                return items;
        }
    }

    const onFilterProject = (filter) => {
        setFilter(filter);
    }
